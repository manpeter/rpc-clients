...
trait ServiceTrait
{
    ...
    protected function __proxyCall(string $interfaceClass, string $methodName, array $params)
    {
        ...
        try {
            ...
            //如果熔断开启,直接降级
            if ($state == CircuitBreak::StateOpen) throw  new RpcClientException("Rpc CircuitBreak:" . $fallbackName->$methodName()[0]);
            //半开状态,是允许访问后台服务的
            if ($state == CircuitBreak::StateHalfOpen) {
                ...
                throw  new RpcClientException("Rpc CircuitBreak:" . $fallbackName->$methodName()[0]);
            }
            ...
            if (stristr($message, "Rpc call") || stristr($message, "Create connection error") || stristr($message,"Connect failed host")) {
                ...
                if (CircuitBreak::StateClose == $state) {
                    ...
                    throw  new RpcClientException("Rpc CircuitBreak:" . $fallbackName->$methodName()[0]."--正常熔断");
                }
                ...
                if (CircuitBreak::StateHalfOpen == $state) {
                    ...
                    $circuit->OpenBreaker($address);//开启熔断器,记录了延迟时间
                    echo "半开状态重置" . PHP_EOL;
                    throw  new RpcClientException("Rpc CircuitBreak:" . $fallbackName->$methodName()[0]."---半开熔断");
                }

                //如果当前熔断是开启状态并且时连接失败的异常
                if (CircuitBreak::StateOpen == $state && stristr($message, "Create connection error")) {
                     throw  new RpcClientException("Rpc CircuitBreak:" . $fallbackName->$methodName()[0]."---连接熔断");
                }
            }
